...
LOGO_PATH = os.path.join(BASE_DIR, "Agora logo.png")
OUTPUT_FOLDER = os.path.join(BASE_DIR, "bills")
LOG_FOLDER = os.path.join(BASE_DIR, "logs")
CSV_LOG_PATH = os.path.join(LOG_FOLDER, "Bill_Generating_Log_Summary.csv")
...
# History table (must already exist)
HISTORY_TABLE = "dbo.billing_history"

# Toggle: skip invoices already present in history (by invoice_number)
    ...
    if not os.path.exists(CSV_LOG_PATH) or os.path.getsize(CSV_LOG_PATH) == 0:
        with open(CSV_LOG_PATH, mode='w', newline='') as file:
            ...
            writer.writerow([
                ...
                "Bills Generated", "Errors Encountered", "Skipped Duplicates"
            ])

logging.basicConfig(
    filename=os.path.join(LOG_FOLDER, "Bill Generating Log.log"),
    filemode='a',
...
# Charts
# =========================
def generate_pie_chart(breakdown_series: pd.Series, output_path: str, custom_colors_map: Dict[str, str]) -> None:
    colors = [custom_colors_map.get(cat, "#2A3240") for cat in breakdown_series.index]
    ...
    for i, autotext in enumerate(autotexts):
        ...
        ax.text(x + 0.04, y, percent, color="black", fontsize=9, ha="left", va="center")
    fig.legend(wedges, breakdown_series.index, loc="lower center",
...
def main() -> None:
    ...
    print(f"[DEBUG] LOGO_PATH exists? {os.path.exists(LOGO_PATH)}")
    ...
    print(f"[DEBUG] DB_PASSWORD={'***' if password else None}")

    logging.warning(f"🚀 bill_generator.main() started | BASE_DIR={BASE_DIR}")
    logging.warning(f"📄 LOGO_PATH={LOGO_PATH}")
    logging.warning(f"📂 OUTPUT_FOLDER={OUTPUT_FOLDER}")
    logging.warning(f"📂 LOG_FOLDER={LOG_FOLDER}")
    ...
    logging.info("=============== Bill Generation Start ===============")
    ...
    for _, inv in headers.iterrows():
        ...
        multi_mirn_path: Optional[str] = None

        try:
            # Base invoice number from monthly
            invoice_no = inv.get("invoice_number")

            # Detail lines for this invoice (breakdown view)
            invoice_lines = breakdown[breakdown["invoice_number"] == invoice_no].copy()
            charges_df = build_charges_df_from_breakdown(invoice_lines)

            if charges_df.empty:
                logging.info(f"No charge lines found for invoice {invoice_no}; skipping PDF and history.")
                continue

            # Prepare history row and decide increment/skip BEFORE generating the PDF
            hist_row = build_history_row_from_monthly(inv)
            r2, invoice_no, do_insert = _apply_history_increment_rule(eng, hist_row)

            if not do_insert:
                skipped_duplicates += 1
                logging.info(f"Duplicate invoice (same total) detected: {invoice_no}; skipping PDF and history.")
                continue
            ...
            # Category totals for pie/list (from breakdown)
            cat_order = ["Firm Gas Sales","Spot Gas Sales","Transport Fee","Distribution Charges","Adjustment Charges","Other Charges"]
            s_for_pie_cat = (
                charges_df.groupby("Charge Category")["Amount ex GST"]
                          .apply(lambda s: pd.to_numeric(s, errors="coerce").fillna(0.0).sum())
            )
            try:
                s_for_pie_cat = s_for_pie_cat.reindex(cat_order).dropna()
            except Exception:
                pass
            s_for_pie_cat = s_for_pie_cat[s_for_pie_cat > 0]

            # Daily consumption (from daily view)
            ...
            if not cust_consumption.empty:
                cust_consumption["gas_date"] = pd.to_datetime(cust_consumption["gas_date"], errors="coerce")
                cust_consumption["gj_consumption"] = pd.to_numeric(cust_consumption["gj_consumption"], errors="coerce")
                cust_consumption = cust_consumption.dropna(subset=["gas_date"]).sort_values("gas_date")

                # contract MDQ (unchanged)
                if "retail_mdq" in cust_consumption.columns and not cust_consumption["retail_mdq"].isna().all():
                    contract_mdq = float(cust_consumption["retail_mdq"].dropna().iloc[0])

                _vals = cust_consumption["gj_consumption"].dropna()
                monthly_mdq = float(_vals.max()) if not _vals.empty else None

                consumption_path = os.path.join(OUTPUT_FOLDER, f"bar_{mirn}_{GENERATE_DATE}.png")
                consumption_ok = generate_consumption_chart(
                    mirn=str(mirn),
            ...
            def write_label_value(
                ...
                if wrap or force_two_lines:
                    ...
                    if move_down:
                        # approximate two lines if forced; otherwise leave where multi_cell ended
                        extra = (2 * line_height) if force_two_lines else 0
                        pdf.set_y(max(pdf.get_y(), current_y + extra))
                else:
                    if value_w:
            ...
            pdf.ln(14)
            write_label_value("Total Amount Payable", f"${invoice_amount:,.2f}",
                              x=right_start_x, label_w=label_w_right, value_w=value_w_right,
                              align="R", bold_size=10, reg_size=10.5)
            ...
            # pie (right)
            if not s_for_pie_cat.empty and float(s_for_pie_cat.sum()) > 0:
                pie_path = os.path.join(OUTPUT_FOLDER, f"pie_{acct}_{GENERATE_DATE}.png")
                generate_pie_chart(s_for_pie_cat, pie_path, custom_colors)
                if os.path.exists(pie_path):
                    pdf.image(pie_path, x=115, y=123.5, w=80)
            ...
            if pd.notna(acct_num) and int(acct_num) in accounts:
                multi_mirn_path = os.path.join(OUTPUT_FOLDER, f"bar_accounts_{GENERATE_DATE}.png")
                ...
                if not daily.empty and need_cols.issubset(daily.columns):
                    ...
                    if pd.notna(start_date_dt) and pd.notna(end_date_dt):
                    ...
                    if not acc_df.empty:
                        ...
                        multi_mirn_ok = generate_accounts_mirn_chart(
                            billing_period=billing_period,
                            df=acc_df,
                            output_path=multi_mirn_path
                        )

            CHART_W_MM      = 180.0
            CHART_FULL_H_MM = 64.0
            CHART_HALF_H_MM = 32.0
            want_half = bool(multi_mirn_ok and multi_mirn_path and os.path.exists(multi_mirn_path))
            ...
            if consumption_ok and os.path.exists(consumption_path):
                _ensure_space(first_h_mm + 2)
                pdf.image(consumption_path, x=10, w=CHART_W_MM)

            if multi_mirn_ok and os.path.exists(multi_mirn_path):
                second_h_mm = CHART_HALF_H_MM
                second_box_h_mm = 8

                _ensure_space(second_box_h_mm + 2 + second_h_mm + 2)
                ...
                pdf.rect(10, current_y, 70, second_box_h_mm)
                pdf.ln(1)

                pdf.image(multi_mirn_path, x=10, w=CHART_W_MM)
            ...
            # Save PDF (auto-increment filename for files in OUTPUT_FOLDER)
            prefix = f"{invoice_no}_"
            existing = [
                name for name in os.listdir(OUTPUT_FOLDER)
                if name.startswith(prefix) and name.lower().endswith(".pdf")
            ]
            if existing:
                filename = f"{invoice_no}_{len(existing) + 1}_Generated_{GENERATE_DATE}.pdf"
            else:
                filename = f"{invoice_no}_Generated_{GENERATE_DATE}.pdf"

            out_path = os.path.join(OUTPUT_FOLDER, filename)
            pdf.output(out_path)

            # Log history ONLY when PDF was successfully written
            history_rows.append(r2)  # r2 is the potentially suffixed row to insert
            logging.info(f"Prepared history rows so far: {len(history_rows)}")

            # Cleanup images
            for pth in (consumption_path, pie_path, multi_mirn_path):
            ...
            bill_count += 1
            logging.info(f"Bill generated: {filename}")

        except Exception as e:
            error_count += 1
            logging.exception(f"Failed to generate bill for invoice {inv.get('invoice_number','?')}: {e}")
            try:
                if pdf:
                    debug_name = f"DEBUG_{inv.get('account_number','?')}_{GENERATE_DATE}.pdf"
                    pdf.output(os.path.join(OUTPUT_FOLDER, debug_name))
            except Exception:
                pass
    ...
    # Write billing history once (only successful PDFs were queued)
    try:
        print("\n==== DEBUG: Billing History Pipeline ====")
        debug_describe_table(eng, HISTORY_TABLE)
        print(f"[DEBUG] Prepared history_rows: {len(history_rows)}")
        if history_rows:
            debug_preview_history_rows(history_rows, limit=1)
            debug_validate_not_nulls(eng, history_rows[0])
            insert_billing_history_batch(eng, history_rows)
        with eng.begin() as con:
            cnt = con.exec_driver_sql(f"SELECT COUNT(*) FROM {HISTORY_TABLE}").scalar()
        print(f"[DEBUG] Final rowcount in {HISTORY_TABLE}: {cnt}")
        print("==== DEBUG END ====\n")

    except Exception as e:
        logging.exception(f"Failed to insert billing history: {e}")
        print(f"[DEBUG] Exception in billing history pipeline: {type(e).__name__}: {e}")

    # wrap-up logs
    end_time = datetime.now()
    duration = end_time - start_time
    logging.info("=============== Bill Generation Completed ===============")
    logging.info(f"Start Time: {start_time}")
    logging.info(f"End Time:   {end_time}")
    logging.info(f"Duration:   {duration}")
    logging.info(f"Total Bills Generated: {bill_count}")
